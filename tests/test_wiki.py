...
class TestWiki:
    ...
    def test_validate_config(self):
        ...
        with patch.object(
            challengeutils.wiki, "read_wiki_config", return_value=self.wiki_config
        ) as patch_read:
    ...
    def test_validate_config_invalid(self, append_dict, error_message):
        ...
        with pytest.raises(ValueError, match=error_message), patch.object(
            challengeutils.wiki, "read_wiki_config", return_value=self.wiki_config
        ) as patch_read:
            config = challengeutils.wiki.validate_config("./")
            patch_read.assert_called_once_with("./")
    ...
    def test_pull_wiki(self):
        ...
        wiki = synapseclient.Wiki(
            title="Echoes Test", owner="syn22", markdown="test", id="2"
        )
        with patch.object(
            self.syn, "getWikiHeaders", return_value=wiki_headers
        ) as patch_get_headers, patch.object(
            self.syn, "getWiki", return_value=wiki
        ) as patch_get_wiki:
    ...
    def test_push_wiki(self):
        """Testing push wiki"""
        wiki_obj = synapseclient.Wiki(
            title="Echoes", owner="syn22", markdown="test", id="44"
        )
        self.wiki_config.append(
            {"title": "Time", "parentId": "1", "markdown_path": self.file2.name}
        )
        ...
        with patch.object(
            challengeutils.wiki, "read_wiki_config", return_value=self.wiki_config
        ) as patch_read, patch.object(
            self.syn, "store", return_value=wiki_obj
        ) as patch_store:
