...
def pull_wiki(
    syn: Synapse, project: str, workdir: str = "./"
) -> typing.List[dict]:
    ...
    for wiki_header in wiki_headers:
        ...
        clean_title = "".join(
            letter for letter in wiki["title"] if letter.isalnum()
        )
...
def validate_config(workdir: str) -> typing.List[dict]:
    ...
    home_page = [
        wiki_header.get("parentId") is None for wiki_header in wiki_config
    ]
    if sum(home_page) != 1:
        raise ValueError(
    ...
    for wiki_header in wiki_config:
        ...
        if parentid is not None and parentid not in ids:
            raise ValueError(
                "`parentId` must be one of the " "`id`s in the config"
            )
    return wiki_config


def push_wiki(
    syn: Synapse, projectid: str, workdir: str = "./"
) -> typing.List[dict]:
