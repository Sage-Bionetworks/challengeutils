...
def test_valid__check_date_range():
    ...
    result = challengeutils.utils._check_date_range(
        date_str, datetime1, datetime2
    )
    assert result
...
def test_get_contributors():
    ...
    with patch.object(
        ...
        assert all_contributors == {321}


def test_list_evaluations():
    with mock.patch.object(
        syn, "getEvaluationByContentSource"
    ) as patch_geteval:
...
def test_userid__get_submitter_name():
    ...
    with mock.patch.object(
        ...
        submittername = challengeutils.utils._get_submitter_name(
            syn, submitterid
        )
