...
member1=challengeutils.utils.NewUserProfile(ownerId='1234', firstName='test', lastName='foo', userName='temp')
...
def test_team_members_diff():
    assert challengeutils.utils.team_members_diff(syn, 1, 2) == set([member4,member3])

def test_team_members_intersection():
    assert challengeutils.utils.team_members_intersection(syn, 1, 2) == set([member1,member2])

def test_team_members_union():
    assert challengeutils.utils.team_members_union(syn, 1, 2) == set([member1,member2,member3,member4,member5,member6])
