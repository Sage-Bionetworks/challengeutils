...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            synapseclient.Folder(parentId="syn123"),
            None,
            "download",
        ...
        (
            synapseclient.Schema(parentId="syn123"),
            None,
            "edit_and_delete",
            permissions.ENTITY_PERMS_MAPPINGS["edit_and_delete"],
        ),
        (
            synapseclient.File(parentId="syn123"),
            None,
            "admin",
        ...
        (
            synapseclient.Evaluation(contentSource="syn123"),
            None,
            "view",
...
def test__set_permissions(entity, principalid, permission_level, mapped):
    ...
    with patch.object(
        SYN, "setPermissions", return_value=SET_PERMS
    ) as patch_set_permission:
        permissions._set_permissions(
            SYN, entity, principalid=principalid, permission_level=permission_level
        )
        patch_set_permission.assert_called_once_with(
            entity, principalId=principalid, accessType=mapped
        )
...
def test_wrong_permission_level():
    ...
    with pytest.raises(ValueError, match=r"permission_level must be one of these:.*"):
        permissions._set_permissions(
            SYN, synapseclient.Entity(), principalid="3", permission_level="foo"
        )


@pytest.mark.parametrize("entity", ["syn123", synapseclient.Entity(id="syn123")])
