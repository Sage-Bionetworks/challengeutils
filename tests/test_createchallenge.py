...
def test_existing_create_team():
    ...
    ) as patch_get, patch("builtins.input", return_value="y"):
        team = createchallenge.create_team(
            SYN, team_name, desc, can_public_join=can_public_join
        )
        assert team == expected_team
...
def test_livesitenone_main():
    ...
    org_permission_call = mock.call(
        SYN, proj, team_map["team_org_id"], permission_level="download"
    )
    ...
    with patch.object(
        ...
        patch_create_chal.assert_called_once_with(
            SYN, proj, team_map["team_part_id"]
        )
...
def test_livesite_main():
    ...
    with patch.object(
        ...
        components = createchallenge.main(
            SYN, challenge_name, live_site="syn123"
        )
