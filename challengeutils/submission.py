...
WORKFLOW_START_KEY = (
    "orgSagebionetworksSynapseWorkflowOrchestratorExecutionStarted"
)
TIME_REMAINING_KEY = (
    "orgSagebionetworksSynapseWorkflowOrchestratorTimeRemaining"
)
...
def attach_writeup(syn, writeup_queueid, submission_queueid):
    ...
    submissions = list(
        utils.evaluation_queue_query(
            ...
            "where prediction_file_status == 'SCORED'".format(
                submission_queueid
            ),
        )
    )
...
def _validate_ent_type(submission):
    ...
    try:
        if not isinstance(submission.entity, entity.Project):
            ent_type = re.search(
                r"entity\.(.*?)'", str(type(submission.entity))
            ).group(1)
...
def stop_submission_over_quota(
    ...
    runtime = (
        view_querydf[WORKFLOW_LAST_UPDATED_KEY]
