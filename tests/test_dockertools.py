...
class TestDockerRepository:
    ...
    def test__get_bearer_token_url(self):
        ...
        request_header.headers = {
            "Www-Authenticate": '"service"="foo","Bearer realm"="baz","scope"="bar"'
        }  # noqa pylint: disable=line-too-long
        with patch.object(
            requests, "get", return_value=request_header
        ) as patch_get:
    ...
    def test__get_bearer_token_get_token(self):
        ...
        with patch.object(
            self.docker_cls, "_get_bearer_token_url", return_value="testing"
        ) as patch_get_url, patch.object(
            requests, "get", return_value=request_header
        ) as patch_get, patch.object(
            request_header, "json", return_value={"token": self.token}
        ) as patch_json:
            token = self.docker_cls._get_bearer_token(
    ...
    def test__get_bearer_token_raise_error(self):
        ...
        with patch.object(
            self.docker_cls, "_get_bearer_token_url"
        ), patch.object(
            requests, "get", return_value=request_header
        ), patch.object(
            request_header, "json", return_value={"details": "errorme"}
        ), pytest.raises(
            ValueError, match="errorme"
        ):
            self.docker_cls._get_bearer_token(
                username=self.username, password=self.password
            )
    ...
    def test_get(self):
        ...
        request = Mock()
        with patch.object(
            self.docker_cls, "_get_bearer_token", return_value=self.token
        ) as patch_get_token, patch.object(
            requests, "get", return_value=request
        ) as patch_get:
            resp = self.docker_cls.get(
                username=self.username, password=self.password
            )
            assert resp == request
            patch_get_token.assert_called_once_with(
                username=self.username, password=self.password
            )
            patch_get.assert_called_once_with(
                self.request_url, headers=self.auth_headers
            )
    ...
    def test_validate_docker(self):
        ...
        response = Mock()
        with patch.object(
            dockertools, "DockerRepository", return_value=self.docker_cls
        ) as patch_cls, patch.object(
            self.docker_cls, "get", return_value=response
        ) as patch_resp, patch.object(
            dockertools, "check_docker_exists"
        ) as patch_exists, patch.object(
            dockertools, "check_docker_size"
        ) as patch_size:
            ...
            patch_resp.assert_called_once_with(
                username=self.username, password=self.password
            )
...
def test_check_docker_exists_error():
    ...
    with pytest.raises(
        ValueError, match="Docker image and sha digest must exist."
    ):
        dockertools.check_docker_exists(response)
...
def test_check_docker_size_toobig():
    ...
    with patch.object(
        response, "json", return_value={"layers": layer_dict}
    ), pytest.raises(
        ValueError, match="Docker image must be less than a terabyte."
    ):
        dockertools.check_docker_size(response)
