...
def _convert_to_annotation_cls(
    ...
    if is_synapse_annotations(values):
        ...
        values = Annotations(
            id=sub_status.id, etag=sub_status.etag, values=values
        )
    return values
...
def annotate_submission_with_json(
    syn, submissionid, annotation_values, **kwargs
):
    """
    ...
    sub_status = annotate_submission(
        syn, submissionid, annotation_json, **kwargs
    )
    return sub_status
...
def annotate_submission(
    ...
    sub_status = update_submission_status(
        sub_status, annotation_dict, status=status
    )
