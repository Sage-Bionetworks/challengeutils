...
def test_create_thread_reply():
    ...
    with mock.patch.object(
        syn, "restPOST", return_value=REPLY_DICT
    ) as patch_get_obj:
...
def test_copy_reply():
    ...
    with mock.patch.object(
        ...
        patch_create_reply.assert_called_once_with(
            syn, THREAD_OBJ.id, new_reply_text
        )
...
def test_copy_forum():
    ...
    with mock.patch.object(
        ...
        patch_get_threads.assert_called_once_with(syn, PROJECTID)
        patch_copy_thread.assert_called_once_with(
            syn, THREAD_OBJ, new_projectid
        )
