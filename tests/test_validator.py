...
def validator():
    ...
    with patch.object(SYN, "getEvaluation", return_value=EVALUATION):
        validate = EvaluationQueueValidator(SYN, EVALUATION, admin_user_ids=[1, 3])
    return validate
...
def test_valid_notify(validator):
    ...
    with patch.object(messages, "validation_passed") as patch_send, patch.object(
        SYN, "getUserProfile", return_value=SYN_USERPROFILE
    ) as patch_get_user:
...
def test_error_notify(validator):
    ...
    with patch.object(messages, "validation_failed") as patch_send, patch.object(
        SYN, "getUserProfile", return_value=SYN_USERPROFILE
    ) as patch_get_user:
