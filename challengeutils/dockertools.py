...
class DockerRepository:
    ...
    def __init__(
        self, docker_repo: str, docker_digest: str, index_endpoint: str
    ):
        """
        Args:
    ...
    def get_request_url(self):
        """Gets request URL"""
        url = "/".join(
            ["v2", self.docker_repo, "manifests", self.docker_digest]
        )
...
def check_docker_size(docker_resp: Response, size: int = 1000):
    ...
    """
    docker_size = sum(
        [layer["size"] for layer in docker_resp.json()["layers"]]
    )
