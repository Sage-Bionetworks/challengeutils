...
def acquire_lock_or_fail(name, max_age=LOCK_DEFAULT_MAX_AGE):
    ...
    if lock.acquire():
        return lock
    raise LockedException("A lock exists named {} who's age is: {}".format(
        name, lock.get_age()))
...
class Lock(object):
    ...
    def __init__(self, name, dir=None, max_age=LOCK_DEFAULT_MAX_AGE):
        ...
        self.lock_dir_path = os.path.join(
            self.dir, ".".join([name, Lock.SUFFIX]))
        self.max_age = max_age

    def get_age(self):
        age = timedelta(
            seconds=time.time() - os.path.getmtime(self.lock_dir_path))
        return age
    ...
    def acquire(self, break_old_locks=True):
        ...
        try:
            ...
            if break_old_locks and self.get_age() > self.max_age:
                sys.stderr.write("Breaking lock who's age is: {}\n".format(
                    self.get_age()))
                self.held = True
                # Make sure the modification times are correct
                # On some machines, the modification time could be seconds off
