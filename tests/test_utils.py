...
def test_filter__switch_annotation_permission():
    ...
    existing = challengeutils.utils._switch_annotation_permission(
        add_annotations, existing_annotations, force=True
    )
    assert existing == {}
...
def test_topublic_update_single_submission_status():
    ...
    expected_annot = to_submission_status_annotations(add_annotations, is_private=False)
...
def test__get_contributors():
    ...
    sub = synapseclient.Submission(
        ...
        entityId="syn1234",
        versionNumber=1,
        contributors=[{"principalId": 321}],
        createdOn="2019-05-26T23:59:59.062Z",
    )
    bundle = [(sub, "temp")]
    with patch.object(
        syn, "getSubmissionBundles", return_value=bundle
...
def test_get_contributors():
    ...
    with patch.object(
        ...
        all_contributors = challengeutils.utils.get_contributors(syn, ids, "SCORED")
        assert all_contributors == set([321])


def test_list_evaluations():
    with mock.patch.object(syn, "getEvaluationByContentSource") as patch_geteval:
...
def test_defaultloc_download_submission():
    ...
    with mock.patch.object(
        syn, "getSubmission", return_value=submission_dict
    ) as patch_get_submission:
        sub_dict = challengeutils.utils.download_submission(syn, "12345")
        patch_get_submission.assert_called_once_with("12345", downloadLocation=None)
...
def test_specifyloc_download_submission():
    ...
    """
    entity = synapseclient.Entity(versionNumber=4, concreteType="foo", id="syn123")
    submission_dict = {
        "entity": entity,
        "evaluationId": 12345,
        "filePath": "/path/here",
    ...
    with mock.patch.object(
        ...
        patch_get_submission.assert_called_once_with("12345", downloadLocation=".")
        assert sub_dict == expected_submission_dict
...
def test_userid__get_submitter_name():
    ...
    with mock.patch.object(
        syn, "getUserProfile", return_value=userinfo
    ) as patch_get_user, mock.patch.object(syn, "getTeam") as patch_get_team:
        submittername = challengeutils.utils._get_submitter_name(syn, submitterid)
...
def test_teamid__get_submitter_name():
    ...
    teaminfo = {"name": "foo"}
    with mock.patch.object(
        syn, "getUserProfile", side_effect=SynapseHTTPError
    ) as patch_get_user, mock.patch.object(
        syn, "getTeam", return_value=teaminfo
    ) as patch_get_team:
        submittername = challengeutils.utils._get_submitter_name(syn, submitterid)
...
def test_delete_submission():
    ...
    ) as patch_get, patch.object(syn, "delete") as patch_delete:
