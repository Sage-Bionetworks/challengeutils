...
def set_evaluation_permissions(syn, evaluation, principalid=None, permission_level=None):
	"""
	...
	Args:
		...
		permission_level: Can be "view","submit","score", "admin", and None for removal of ACL
	"""
	permission_level = "remove" if permission_level is None else permission_level
	view = ["READ"]
	submit = ['READ','SUBMIT']
	...
	permission_level_mapping = {'remove':[],
								'view':view,
								'submit':submit,
								'score':score,
								'admin':admin}
...
def set_entity_permissions(syn, entity, principalid=None, permission_level=None):
	"""
	Convenience function to set ACL on an entity for a user or team based on
	permission levels (view, download...)
	...
	Args:
		...
		principalid: Identifier of a user or group (defaults to PUBLIC users)
		permission_level: Can be "view","download","edit","edit_and_delete", "admin", and None for removal of ACL
	"""
	permission_level = "remove" if permission_level is None else permission_level
	view = ["READ"]
