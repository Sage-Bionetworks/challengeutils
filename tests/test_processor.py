...
SUBMISSION_STATUS = synapseclient.SubmissionStatus(
    status="RECEIVED", id="111", etag="222"
)
EVALUATION = synapseclient.Evaluation(
    name="foo", id="222", contentSource=CHALLENGE_SYNID
)
...
class Processor(EvaluationQueueProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        EvaluationQueueProcessor.__init__(
            self,
            syn,
            evaluation,
...
def test_init():
    ...
    ) as patch_get_eval, patch.object(
        SYN, "getUserProfile", return_value={"ownerId": 1111}
    ) as patch_getuser:
...
def test_specifyadmin_init():
    ...
    with patch.object(
        SYN, "getEvaluation", return_value=EVALUATION
    ) as patch_get_eval, patch.object(SYN, "getUserProfile") as patch_getuser:
        proc = Processor(
            SYN,
            EVALUATION,
            admin_user_ids=[1, 3],
...
def test_invalid_store_submission_status(processor):
    ...
    info["valid"] = False
    with patch.object(
        scoring_harness.base_processor,
        "update_single_submission_status",
        return_value=status,
    ) as patch_update, patch.object(SYN, "store") as patch_store:
        processor.store_submission_status(SUBMISSION_STATUS, info)
        patch_update.assert_called_once_with(
            SUBMISSION_STATUS, SUB_INFO["annotations"], is_private=False
        )
...
def test_default_call(processor):
    ...
    with patch.object(
        ...
        processor, "interact_with_submission", return_value=SUB_INFO
    ) as patch_interact, patch.object(
        processor, "store_submission_status"
    ) as patch_store, patch.object(
        processor, "notify"
    ) as patch_notify:
        processor()
        patch_get_bundles.assert_called_once_with(EVALUATION, status="RECEIVED")
...
def test_removecache_call(processor):
    ...
    with patch.object(
        ...
        scoring_harness.base_processor, "_remove_cached_submission"
    ) as patch_remove, patch.object(
        processor, "notify"
    ) as patch_notify:
        processor()
        patch_get_bundles.assert_called_once_with(EVALUATION, status="RECEIVED")
