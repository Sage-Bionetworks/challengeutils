...
def command_set_entity_acl(syn, args):
    permissions.set_entity_permissions(
        syn, args.entityid, principalid=args.principalid,
        permission_level=args.permission_level)


def command_set_evaluation_acl(syn, args):
    permissions.set_evaluation_permissions(
        syn, args.evaluationid, principalid=args.principalid,
        permission_level=args.permission_level)
...
def build_parser():
    ...
    parser_createChallenge = subparsers.add_parser(
        'createchallenge', help='Creates a challenge from a template')
    parser_createChallenge.add_argument(
        'challengename', help='Challenge name')
    parser_createChallenge.add_argument(
        '--livesiteid', help='Option to specify the live site synapse ID '
    ...
    parser_mirrorWiki = subparsers.add_parser(
        'mirrorwiki', help=(
            'This command mirrors wiki pages. It relies on the wiki titles '
    ...
    parser_mirrorWiki.add_argument(
        'entityid', type=str, help=(
            'Synapse ID of the project\'s wiki changes you have staged'))
    parser_mirrorWiki.add_argument(
        'destinationid', type=str, help=(
            'Synapse ID of project whose wiki you want to update with the '
            'entityid'))
    parser_mirrorWiki.add_argument(
        '--forceupdate', action='store_true', help=(
            'Update the wikipages even if they are the same'))
    ...
    parser_query.add_argument(
        'uri', type=str, help=(
            "Synapse ID of the project's wiki you want to copy"))
    parser_query.add_argument(
        '--limit', type=int, help=(
            'How many records should be returned per request'),
        default=20)
    parser_query.add_argument(
        '--offset', type=int, default=0, help=(
    ...
    parser_change_status = subparsers.add_parser(
        'changestatus', help='Changes the status of a submission ID')
    ...
    parser_attach_writeup = subparsers.add_parser(
        'attachwriteup', help=(
            'Attaches the write ups of a challenge to its main challenge '
            'queue'))
    parser_attach_writeup.add_argument(
        'writeupqueue', type=str, help=(
            'Write up submission queue evaluation ID'))
    parser_attach_writeup.add_argument(
        'submissionqueue', type=str, help=(
            'Challenge submission queue evaluation ID'))
    parser_attach_writeup.set_defaults(func=command_writeup_attach)

    parser_set_entity_acl = subparsers.add_parser(
        'setentityacl', help='Sets the permissions of a Synapse Entity')
    ...
    parser_set_entity_acl.add_argument(
        'permission_level', type=str, help='Permissions',
    ...
    parser_set_evaluation_acl = subparsers.add_parser(
        'setevaluationacl', help=(
            'Sets the permissions of a Synapse Evaluation Queue'))
...
def synapseLogin():
    try:
        ...
        print('Please provide your synapse username/email and password '
